...
def test_reconcile_worktrees_lists_allocated_directories(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    ...
    payload = MODULE.reconcile_worktrees(worktree_root)
    assert payload["status"] == "ok"
    assert allocated["path"] in payload["worktrees"]
